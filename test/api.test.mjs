...
function makeToken(sn) {
  return 'Bearer ' + Buffer.from(sn + ':placeholder').toString('base64');
}
...
// ═══════════════════════════════════════════════════════════════
// 8. Auth tests (TOKEN_AUTH=true)
// ═══════════════════════════════════════════════════════════════
describe('Auth required (TOKEN_AUTH=true)', () => {
  let authApp;
  let req;

  beforeAll(async () => {
    process.env.TOKEN_AUTH = 'true';
    vi.resetModules();

    // Clear Node module cache for all server modules so config re-reads env
    for (const key of Object.keys(Module._cache)) {
      if (key.startsWith(serverDir + path.sep)) {
    ...
    const freshRequire = createRequire(path.join(serverDir, 'app.js'));
    authApp = freshRequire('./app.js');
    req = supertest(authApp);
  });

  afterAll(() => {
    delete process.env.TOKEN_AUTH;
  });

  it('401 — upload without Bearer token', async () => {
    const res = await req
      .post('/api/v1/books/upload')
      .field('sn', VALID_SN)
      .attach('file', bookContent, 'no-auth.txt')
      .expect(401);

    expect(res.body.error).toMatch(/token/i);
  });

  it('200 — upload with valid Bearer token', async () => {
    const res = await req
      .post('/api/v1/books/upload')
      .field('sn', VALID_SN)
      .attach('file', bookContent, 'has-auth.txt')
      .set('Authorization', makeToken(VALID_SN))
      .expect(200);

    expect(res.body).toHaveProperty('book_id');
  });

  it('401 — batch-upload without Bearer token', async () => {
    const res = await req
      .post('/api/v1/books/batch-upload')
      .field('sn', VALID_SN)
      .attach('files', Buffer.from('x'), 'no-auth-batch.txt')
      .expect(401);

    expect(res.body.error).toMatch(/token/i);
  });

  it('200 — batch-upload with valid Bearer token', async () => {
    const res = await req
      .post('/api/v1/books/batch-upload')
      .field('sn', VALID_SN)
      .attach('files', Buffer.from('x'), 'has-auth-batch.txt')
      .set('Authorization', makeToken(VALID_SN))
      .expect(200);
  ...
  it('403 — token SN does not match body SN', async () => {
    const res = await req
      .post('/api/v1/books/upload')
      .field('sn', 'OTHER01')
      .attach('file', bookContent, 'mismatch.txt')
      .set('Authorization', makeToken(VALID_SN))
      .expect(403);
